...
public final class Dismissal {
    ...
    Dismissal(
            ...
            Player batter,
            @JsonProperty("bowler")
            @Nullable Player bowler,
            @JsonProperty("fielder")
            @Nullable Player fielder
    ) {
    ...
    public @Nonnull String toScorecardString(@Nullable LineUp team) {
        ...
        if (this.fielder == null) {
            ...
            if (team != null) {
                if (team.wicketKeeper() != null && requireNonNull(team.wicketKeeper()).samePlayer(this.fielder)) {
        ...
        return switch (type) {
            ...
            case CAUGHT -> {
                ...
                yield cab ? "c & b " + fielderName : "c " + fielderName + " b " + bowlerName;
            }
            case HIT_WICKET -> "hw " + bowlerName;
            case LEG_BEFORE_WICKET -> "lbw b " + bowlerName;
            case RUN_OUT -> fielderName == null ? "run out" : "run out (" + fielderName + ")";
            case STUMPED -> "st " + fielderName + " b " + bowlerName;
            default -> type.toString().toLowerCase().replace('_', ' ');
        };
    }
