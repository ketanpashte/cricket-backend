...
public final class ImmutableList<T> implements Iterable<T> {
    ...
    public ImmutableList(
        @JsonProperty("arrayList") @Nonnull List<T> arrayList,
        @JsonProperty("first") @Nonnegative int first,
    ...
    public @Nonnull ImmutableList<T> subList(int firstIndex, int lastIndex) {
        ...
        if (newLast > last)
            throw new IllegalArgumentException("The lastIndex value " + lastIndex + " is too large. Max value allowed is " + (size() - 1));
