...
import static org.example.Crictils.stateGuard;
import static org.example.ImmutableList.emptyList;
import static org.example.ImmutableList.toImmutableList;
...
public final class Innings {
    ...
    private Innings(InningsStartingEvent data, Score score, ImmutableList<Partnership> partnerships,
            @Nullable BatterInnings currentStriker, @Nullable BatterInnings currentNonStriker,
            ImmutableList<BatterInnings> batters, ImmutableList<Player> yetToBat, ImmutableList<Over> completedOvers,
            @Nullable Over currentOver, @Nullable Instant endTime, Balls balls,
            ImmutableList<BowlerInnings> bowlerInningses, State state, @Nullable Integer maxOvers,
            @Nullable Integer maxBalls, @Nullable Integer target) {
        this.score = score;
    ...
    static @Nonnull Innings newInnings(InningsStartingEvent event) {
        return new Innings(event, event.startingScore(), emptyList(), null, null, emptyList(),
                event.battingTeam().battingOrder(),
    ...
    public @Nonnull Innings onEvent(MatchEvent event) {
        ...
        if (event instanceof BallCompletedEvent) {
            ...
            if (currentStriker() == null || currentNonStriker() == null)
                throw new IllegalStateException("There is only one batter in. Make sure an "
                        + OverStartingEvent.class.getSimpleName()
                        + " has been raised at the beginning of the over and that a "
            ...
            stateGuard(currentOver == null || !currentOver.isComplete(),
                    () -> "Because the last ball of the innings completed an over, an overCompleted event should be sent before ending the innings");
            ...
            if (striker != null || nonStriker != null) {
                Partnership newPartnership = Partnership.newPartnership(partnerships.size() + 1,
                        striker == null ? nonStriker.player() : striker.player(), newBatterInnings.player(), e.time());
            ...
            if (striker == null) {
                ...
                throw new IllegalStateException(
                        "A new batter innings cannot start when there are already two batters in. Did you forget to add a MatchEvents.batterInningsCompleted()?");
            }
        }
        ...
        return new Innings(data, newScore, partnerships, striker, nonStriker, batters, yetToBat, completedOvers,
                currentOver, endTime, balls, bowlerInningses, newState, maxOvers, maxBalls, target);

    }

    private ImmutableList<BatterInnings> replaceBatterInnings(ImmutableList<BatterInnings> list,
            BatterInnings newValue) {
    ...
    private ImmutableList<BowlerInnings> replaceBowlerInnings(ImmutableList<BowlerInnings> list,
            BowlerInnings newValue) {
    ...
    /**
     * @return The over being bowled, or null if between overs or before/after the
     *         innings has started
     * @see #completedOvers()
     * @see #overs()
     */
    ...
    /**
     * @return The batter currently facing. This may be null before the innings
     *         starts or directly after a dismissal.
     */
    ...
    /**
     * @return The batter at the non-facing end. This may be null before the innings
     *         starts or directly after a dismissal.
     */
    ...
    /**
     * @return All the batters who have batted so far (including batters who are out
     *         and not-out) in the order they
     *         came in.
     */
    ...
    /**
     * @return The players who have not yet batted, in the order they are expected
     *         to bat.
     */
    ...
    /**
     * @return The score the innings started at, which is {@link Score#EMPTY} unless
     *         penalties were conceded by
    ...
    /**
     * @return Gets the info on the bowlers who have bowled so far (in the order
     *         they first bowled in)
     */
    ...
    /**
     * @return The score in the innings so far. Access {@link Score#teamRuns()} for
     *         the total number of runs in this innings.
     */
    ...
    /**
     * @return True if all of the batters (except one) have been dismissed, and
     *         there are no retired batters
    ...
    /**
     * Calculates the Required Run Rate (RRR) if this innings has a target.
     *
     * @return The required run rate, or null if it cannot be calculated.
     */
    public @Nullable RPO requiredRunRate() {
        if (target == null)
            return null; // No target → RRR not applicable
        Integer ballsRemaining = numberOfBallsRemaining();
        if (ballsRemaining == null || ballsRemaining == 0)
            return null;

        int runsRequired = target - score.teamRuns();
        if (runsRequired <= 0)
            return RPO.fromDouble(0.0); // Target achieved

        double oversRemaining = ballsRemaining / 6.0;
        return RPO.fromDouble(runsRequired / oversRemaining);
    }

    /**
     * @return The number of scheduled balls remaining in the innings, or null if
     *         this innings does not have a limit.
     */
    ...
    /**
     * Gets the number of the last ball as a string in the format
     * <em>over.ball</em>, for example &quot;0.1&quot;
     * <p>
     * After 6 balls but before an {@link OverCompletedEvent} it will return a value
     * such as &quot;0.6&quot;
     * while after the over is completed it will be &quot;1.0&quot;
     * </p>
     *
    ...
    /**
     * @return The target the batting team is aiming for in order to win, or null if
     *         this is not the last innings
     *         in the match
     */
    ...
    /**
     * @return 1 for the first innings that the current batting team is batting for
     *         in a match; 2 for the second etc
     */
    ...
    /**
     * @return The time the innings ended, if {@link #state()} is
    ...
    /**
     * @return The current total number of scheduled balls in the innings, after
     *         adjustments such as reductions due
    ...
    /**
     * @return The current total number of scheduled overs in the innings, after
     *         adjustments such as reductions due
     *         to bad weather. (Returns null if there is no limit)
    ...
    /**
     * @return The number of scheduled balls at the beginning of the innings, or
    ...
    /**
     * @return The number of scheduled overs in this match, or null if there is no
     *         limit
     */
    ...
    /**
     * An innings isn't officially complete until an {@link InningsCompletedEvent}
     * is created, however this method can
     * be used to guess if an innings should be ended based on things like remaining
     * wickets or runs or overs to bowl.
     *
     * @return true if the innings is marked as complete, or there are no balls left
     *         (in a limited overs match), or the
     *         target has been reached, or there are no more batters.
     */
    public boolean shouldBeComplete() {
        return (state() == State.COMPLETED) // marked complete
